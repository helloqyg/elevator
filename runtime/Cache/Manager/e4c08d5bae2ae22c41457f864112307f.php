				...
				<div class="row">
					<div class="col-md-12">
						<table class="table table-hover news-list">
						  ...
						  <tbody>
							<?php if(is_array($invoice)): $i = 0; $__LIST__ = $invoice;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($i % 2 );++$i;?><tr>
							  <td><?php echo ($i["company_name"]); ?></td>
							  <td><?php echo ($i["total"]); ?>元</td>
							  <td><?php if(($i["status"]) == "0"): ?>否<?php else: ?>是<?php endif; ?></td>
							  <td class="text-center"><?php echo (date("Y-m-d",$i["time"])); ?></td>
							  <td class="text-center">
								<a class="btn btn-info" href="<?php echo U('invoice/show',array('id'=>$i['id']));?>" role="button">操作</a>
							   </td>
