	...
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title"><?php if($_GET['id']!= '' ): ?>修改<?php else: ?>添加<?php endif; ?>产品 -<span style="font-weight: bold;"><?php echo ($goods_name["goods_name"]); ?> -<?php echo ($address_name["ar_encode"]); ?></span>	 &nbsp; &nbsp; &nbsp; &nbsp;
			<button class="	btn btn-inverse go_1">返回上一步 </h3>
			</div>
			<div class="panel-body">
				<form role="form" class="form-horizontal" method="post" action="">
					<div class="form-group">
						<label class="col-sm-2 control-label" for="field-1">编号</label>	
						<div class="col-sm-6">
							<input type="text" class="form-control" name="ar_encode" id="keywords" value="<?php echo ((isset($data["ar_encode"]) && ($data["ar_encode"] !== ""))?($data["ar_encode"]):''); ?>" placeholder="">
							<p class="help-block"> 如W1，W2，W3..</p>
						</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="field-1">宽度</label>	
						<div class="col-sm-6">
							<input type="text" class="form-control" name="ar_width" id="keywords" value="<?php echo ((isset($data["ar_width"]) && ($data["ar_width"] !== ""))?($data["ar_width"]):''); ?>" placeholder="">
							<p class="help-block"> 单位：米</p>
						</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="field-1">高度</label>	
						<div class="col-sm-6">
							<input type="text" class="form-control" name="ar_height" id="keywords" value="<?php echo ((isset($data["ar_height"]) && ($data["ar_height"] !== ""))?($data["ar_height"]):''); ?>" placeholder="">
							<p class="help-block"> 单位：米</p>
						</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="field-1">预付款</label>	
						<div class="col-sm-6">
							<input type="text" class="form-control" name="ar_price" id="keywords" value="<?php echo ((isset($data["ar_price"]) && ($data["ar_price"] !== ""))?($data["ar_price"]):''); ?>" placeholder="">
							<p class="help-block"> 用户预订费</p>
						</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group">
						...
						<div class="col-sm-6">
							<input type="text" class="form-control" name="ar_price_key" id="keywords" value="<?php echo ((isset($data["ar_price_key"]) && ($data["ar_price_key"] !== ""))?($data["ar_price_key"]):''); ?>" placeholder="">
							<p class="help-block"> 广告实际价格</p>
						</div>
					</div>
					<?php if($_GET['id']!= '' ): ?><div class="form-group">
						<div class="form-group">
						<label class="col-sm-2 control-label" for="field-5">展位状态</label>
							<div class="col-sm-6">
							<label class="radio-inline">
								<input type="radio" name="ar_status" value="2" <?php if(($data["ar_status"]) == "2"): ?>checked<?php endif; ?> /> 内部预订
							</label>
							<label class="radio-inline">
								<input type="radio" name="ar_status" value="1" <?php if(($data["ar_status"]) == "1"): ?>checked<?php endif; ?>/> 已售出
							</label>
							<label class="radio-inline">
								<input type="radio" name="ar_status" value="0" <?php if(($data["ar_status"]) == "0"): ?>checked<?php endif; ?>/> 正常售卖
							</label>
							
							<p class="help-block" style="color:red"> 注意：如果不是【内部预定】，切勿修改此项！有其他需求请联系技术部</p>
						</div>
					</div>
					</div><?php endif; ?>
					<div class="form-group">
						<div class="form-group">
						<!-- <label class="col-sm-2 control-label" for="field-5">是否可以重复购买</label>
							<div class="col-sm-6">
							<label class="radio-inline">
								<input type="radio" name="is_repeat_buy" value="0" <?php if(($data["is_repeat_buy"]) == "2"): ?>checked<?php endif; ?> /> 不可以
							</label>
							<label class="radio-inline">
								<input type="radio" name="is_repeat_buy" value="1" <?php if(($data["is_repeat_buy"]) == "1"): ?>checked<?php endif; ?>/> 可以
							</label>
							
							<p class="help-block" style="color:red"> 注意：现场广告不可以重复购买</p>
						</div> -->
					</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group text-center">
						<input type="hidden" name="ar_id" value="<?php echo ((isset($_GET['id']) && ($_GET['id'] !== ""))?($_GET['id']):''); ?>" />
						<input type="hidden" name="goods_id" value="<?php echo ((isset($_GET['goods_id']) && ($_GET['goods_id'] !== ""))?($_GET['goods_id']):''); ?>" />
						<button type="submit" class="btn btn-info btn-single pull-center ajax-post "  target-form="form-horizontal"><?php if($_GET['id']!= '' ): ?>修改<?php else: ?>添加<?php endif; ?>产品</h3>
					</div>
				</form>
			</div>
		</div>
