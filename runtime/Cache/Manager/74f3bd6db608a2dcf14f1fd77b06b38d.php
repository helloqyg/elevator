				...
				<div class="row">
					<div class="col-md-12">
						...
						<table class="table table-hover news-list">
						  <thead>
							<tr>
							  <th class="col-md-2 text-center">展馆号</th>
							  <th class="col-md-3 text-center">展位图</th>
							  <th class="col-md-3 text-center">展馆类型</th>
							  <th class="col-md-3 text-center">修改时间</th>
							  <th class="col-md-3 text-center">状 态</th>
						  ...
						  <tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$single): $mod = ($i % 2 );++$i;?><tr>
							  ...
							  <a href="<?php echo U('Chart/booth',array('id'=>$single['id']));?>"><?php echo ($single["hall_num"]); ?>馆</a></th>
							  <td class="text-center">
							  <a class="example-image-link" href="<?php echo ($single["pic"]); ?>" data-lightbox="example-1">
								<img class="example-image" src="<?php echo ($single["pic"]); ?>" alt="image-1"  height="20px"/>
										</a>
								</td>
							 <td class="text-center">
							 <?php echo get_chart_type($single['hall_type']);?>
							 </td>
							  <td class="text-center"><?php echo (date("Y-m-d",$single["update_time"])); ?></td>
							  <td class="text-center"><?php if(($single["status"]) == "1"): ?>启用<?php else: ?>未启用<?php endif; ?></td>
							  <td class="text-center">
								<a class="btn btn-info" href="<?php echo U('Chart/edit',array('id'=>$single['id']));?>" role="button">修改</a>
...
$(function(){
	$(".add-single").click(function(){
		window.location.href="<?php echo U('Chart/add');?>";
	})
})
</script>
