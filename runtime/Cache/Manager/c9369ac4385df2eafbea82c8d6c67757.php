						...
						<table class="table table-hover news-list">
						  ...
						  <tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$single): $mod = ($i % 2 );++$i;?><tr>
							  ...
							  <a class="example-image-link" href="<?php echo ($single["images"]); ?>" data-lightbox="example-1">
								<img class="example-image" src="<?php echo ($single["images"]); ?>" alt="image-1"  height="20px"/>
										</a>
								</td>
							  <td class="text-center"><?php echo (date("Y-m-d",$single["add_time"])); ?></td>
							  <td class="text-center"><?php if(($single["status"]) == "1"): ?>未售<?php else: ?>售出<?php endif; ?></td>
							  <td class="text-center">
								<a class="btn btn-info" href="<?php echo U('Adver/add',array('id'=>$single['id']));?>" role="button">修改</a>
...
$(function(){
	$(".add-single").click(function(){
		window.location.href="<?php echo U('Adver/add');?>";
	})
})
</script>
