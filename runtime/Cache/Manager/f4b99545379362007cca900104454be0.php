						...
						<table class="table table-hover news-list">
						  ...
						  <tbody>
							<?php if(is_array($apply)): $i = 0; $__LIST__ = $apply;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($i % 2 );++$i;?><tr>
							  <td>
							 	<a href="<?php echo U('User/verify',array('cid'=>get_uid_cid($i['uid'])));?>"><?php echo ($i["company_name"]); ?></a></td>
							  <td ><?php echo ($i["email"]); ?></td>
							  <td ><?php echo ($i["detail"]); ?></td>
							  <td class="text-center" style="width: 50px"><?php if(($i["status"]) == "2"): ?>完成<?php else: ?>未完成<?php endif; ?></td>
							  <td class="text-center">
								<a class="btn btn-info" href="<?php echo U('chart/ashow',array('id'=>$i['id']));?>" role="button">操作</a>
							   </td>
