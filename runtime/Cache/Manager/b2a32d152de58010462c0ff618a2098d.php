						...
						<table class="table table-hover news-list">
						  ...
						  <tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$news): $mod = ($i % 2 );++$i;?><tr>
							  ...
							  <td class="text-center"><?php echo (date("Y-m-d",$news["atime"])); ?></td>
							  <td class="text-center">
								<a class="btn btn-info" href="<?php echo U('News/edit',array('id'=>$news['id']));?>" role="button">修改</a>
								<a class="btn btn-warning" href="javascript:void(0);" onClick="del(<?php echo ($news["id"]); ?>)" role="button">删除</a>
							   </td>
