...
function myrefresh(){window.location.reload();}
	...
	function del(aid){
		...
		$.get("<?php echo U('Adver/address_del');?>", {id:aid},function(data){
			if(data.status == 1){
...
<div class="row">
	<div class="col-md-12">
		<div class="panel panel-default">
			...
			<span style="font-weight: bold;"><?php echo ($addressName["goods_name"]); ?></span>	 &nbsp; &nbsp; &nbsp; &nbsp;
						  ...
						  <tbody>
							<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							  <th scope="row" class="text-center">
								<?php echo ($v["ar_encode"]); ?>
							  </th>
							  <td scope="row" class="text-center">
								<?php echo ($v["ar_price"]); ?>
							  </td>
							 <td scope="row" class="text-center">
								<?php echo ($v["ar_price_key"]); ?>
							  </td>
							  <td class="text-center"> 
							  <?php if(($v["ar_status"]) == "1"): ?><span style="color:red">已售</span><?php endif; ?>
							  <?php if(($v["ar_status"]) == "0"): ?>未售<?php endif; ?>
							  <?php if(($v["ar_status"]) == "2"): ?><span style="color:blue">内部预定</span><?php endif; ?>

							  </td>
							  <td class="text-center">
								<a class="btn btn-info" href="<?php echo U('Adver/change_address',array('id'=>$v['ar_id'],'goods_id'=>I('get.id')));?>" role="button">修改</a>
								<a class="btn btn-warning btn8" href="javascript:void(0);"  attr="<?php echo ($v["ar_id"]); ?>" role="button">删除</a>
							   </td>
...
$(function(){
	$(".add-info").click(function(){
		window.location.href="<?php echo U('Adver/change_address',array('goods_id'=>I('get.id')));?>";
	})
	$('.go_1').click(function(){
		history.go(-1)
	})
