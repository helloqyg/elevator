...
function del(aid){
	...
	$.get("<?php echo U('Chart/del');?>", {id:aid},function(data){
		if(data.status == 1){
...
function update(aid){
	$.get("<?php echo U('Chart/update');?>", {id:aid},function(data){
		if(data.status == 1){
			toastr.success(data.info, opts);
			setTimeout('myrefresh()',2000);
		}else{
						...
						<table class="table table-hover news-list">
						  ...
						  <tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							  <td class="col-md-3 text-center"><?php echo get_company($v['uid']);?></td>
							  <td><?php echo get_hall_name($v['bid']);?>号馆【<?php echo get_booth($v['bid']);?>】</td>
							  <td class="text-center"><?php echo (date("Y-m-d",$v["atime"])); ?></td>
							  <td class="text-center">
								<a class="btn btn-info" onclick="update(<?php echo ($v['id']); ?>)" role="button">通过</a>
								<a class="btn btn-warning" onclick="del(<?php echo ($v['id']); ?>)" role="button">删除</a>
							   </td>
